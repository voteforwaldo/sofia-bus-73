...
export const LIVE_GRACE_MS = 90_000;
export const AT_STOP_THRESHOLD_MS = 3 * 60_000;
...
export function enrichDeparture(departure) {
  ...
  return {
    ...departure,
    // CGM virtual-board times are departure from the stop, not visual arrival.
    arrivalTime: actual,
...
export function getBoardingPhase(departure, now = Date.now()) {
  const remainingMs = departure.arrivalTime - now;

  if (departure.isLive && remainingMs > -45_000 && remainingMs <= AT_STOP_THRESHOLD_MS) {
    return remainingMs <= 0 ? "at_stop_departing" : "at_stop";
  }
...
export function getConfidence({ isLive, delayMinutes, hasGpsEstimate, scheduled, actual }) {
  ...
  if (isLive && typeof scheduled === "number" && typeof actual === "number") {
    return {
      level: "high",
      label: "GPS на живо",
      hint: "Времето е за тръгване от спирката — автобусът може вече да е там",
...
export function filterLine73Departures(departures, matchDirection, now = Date.now()) {
  return departures
    ...
    .filter((departure) => {
      ...
      const graceMs = departure.isLive ? LIVE_GRACE_MS : 60_000;
