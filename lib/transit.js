...
export function getConfidence({ isLive, delayMinutes, hasGpsEstimate, scheduled, actual }) {
  ...
  if (isLive && typeof scheduled === "number" && typeof actual === "number") {
    return {
      level: "high",
      label: "GPS на живо",
      hint: "Активен сигнал от автобуса",
...
export function filterLine73Departures(departures, matchDirection) {
  return departures
    .filter((departure) => departure.lineId === LINE_ID && matchDirection(departure.destination))
    .map(enrichDeparture)
