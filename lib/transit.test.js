      ...
      [
        {
          ...
          tripId: "live-old",
          activeTrip: true,
          time: { scheduled: now - 95_000, actual: now - 95_000 },
          destination: { bg: "ж.к. Младост 1" },
        },
        {
          lineId: "TB39",
...
describe("getBoardingPhase", () => {
  it("detects bus at stop when live and departure is soon", () => {
    const now = 1_000_000;
    expect(
      getBoardingPhase(
        { isLive: true, arrivalTime: now + 90_000 },
        now,
      ),
    ).toBe("at_stop");
  });

  it("detects departing now when live and time passed", () => {
    const now = 1_000_000;
    expect(
      getBoardingPhase(
        { isLive: true, arrivalTime: now - 5_000 },
        now,
      ),
    ).toBe("at_stop_departing");
  });
});
