...
describe("filterLine73Departures", () => {
  ...
  it("drops departures in the past", () => {
    const now = 10_000;
    const departures = filterLine73Departures(
      [
        {
          lineId: "TB39",
          tripId: "old",
          time: { scheduled: now - 120_000, actual: now - 120_000 },
          destination: { bg: "ж.к. Младост 1" },
        },
        {
          lineId: "TB39",
          tripId: "new",
          time: { scheduled: now + 60_000, actual: now + 60_000 },
          destination: { bg: "ж.к. Младост 1" },
        },
      ],
      matchMladost,
    ...
    expect(departures).toHaveLength(1);
    expect(departures[0].tripId).toBe("new");
  });
});
