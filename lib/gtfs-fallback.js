...
export async function fetchGtfsFallbackBoard(stopId, limit = 40) {
  const [tripBuffer, vehicleBuffer] = await Promise.all([
    fetch(TRIP_UPDATES_URL).then((response) => (response.ok ? response.arrayBuffer() : null)),
    fetch(VEHICLE_POSITIONS_URL).then((response) => (response.ok ? response.arrayBuffer() : null)),
  ]);

  if (!tripBuffer && !vehicleBuffer) {
