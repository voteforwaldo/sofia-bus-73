...
export const ROUTES = {
  "to-tokuda": {
    ...
    boardingDirection: "mladost",
    destinationDirection: "mladost",
  ...
  "to-bulgaria": {
    ...
    boardingDirection: "ovchaKupel",
    destinationDirection: "ovchaKupel",
    matchDirection: matchOvchaKupel,
    fallbackRideMinutes: 14,
  },
};

export function getDirectionStopIds(stop, direction) {
  const ids = stop.directionStopIds?.[direction];
  return ids?.length ? ids : stop.stopIds;
}
...
export function buildJourneysForRoute(
  ...
  return boardingDepartures
    ...
    .filter((journey) => {
      const graceMs = journey.isLive ? 90_000 : 60_000;
      return journey.arrivalTime > now - graceMs;
    });
}
